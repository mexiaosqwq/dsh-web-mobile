...
- 侧边栏抽屉手势（#16，PR #37 by @wingsky-1）：屏幕左侧 45% 区域右滑呼出抽屉；抽屉打开后在屏幕任意位置横滑（向左推回或向右拨开都行）即可收起。
- 双向跟手：拉开和收起时抽屉都实时跟着手指走，松手后由宿主 0.28s 过渡从手指位置接着滑到终态。快甩仍可小位移完成开合，中途切后台或弹出系统模态会自动弹回。
...
- 滑动开抽屉偶尔没反应或开了又弹回
- 真机（Android Chrome）贴左缘右滑呼出抽屉会触发浏览器「返回上一页」：根元素 `overscroll-behavior-x: none` 抑制 Chrome 边缘历史导航手势
- 抽屉打开后往左滑收不回去：收起判定此前只认向右拖，且起点必须落在抽屉自身范围内，抽屉右侧那块屏幕完全没有响应
- 拉开抽屉的过程中抽屉不跟手（只在松手后才播动画）
- 左滑收起时抽屉会在半开位置卡住、松手后才补完剩下那截：跟手行程的边界此前按收起态那条窄导航栏的宽度算，比真实抽屉窄 81px
- 左滑松手瞬间抽屉样式突然消失、不是自然动画收起：宿主状态在动画中途被翻转，React 把抽屉整个换成了窄导航栏；现在先播完滑出动画、落地后才切换
- 右滑开始阶段有一小段"真空期"：打开跟手的起点基线深藏在屏幕外 28px，手指要先走完这段才有可见反馈；现在锁轴即跟手、首帧即露边
- 背景黑色遮罩关闭时瞬间消失：补齐渐隐动画，且与抽屉滑出同步进行
- 左缘识别区改为自适应：按视口宽度的 45% 现算（390px 手机约 176px），横竖屏与平板自动跟随，不再是固定像素；此前 48px→96px 两轮加宽解决的「判定太靠左」与浏览器边缘手势重叠问题继续成立
- 起指落在横向滚动容器（状态栏读出条、消息代码块）内时让位给原生滚动，不再误开抽屉
- `?mobile-nav-debug=1` 诊断条在代码重组后没有接线，访问调试参数无任何显示
- 系统开启「减弱动态效果」时抽屉仍播放滑入滑出动画，现与设置面板一致直接禁用

**重构**

- 抽屉手势的左缘识别区改为纯几何判定（96px），移除注入宿主 DOM 的隐形热区元素
