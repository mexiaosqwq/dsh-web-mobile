...
- Shared full-tree reconciler:
  ...
  - Registered tasks: `frame-marker`, `preview-fullscreen-toggle`, `git-chip-reparent`, `settings-toolbar-reparent`, `preview-close-sync`, `sheet-rise-replay`, `stats-line`, `overlay-backdrop-fab`, `sidebar-swipe-hotspot`.
- Effects:
  ...
  - `sidebar-swipe.ts` — drawer swipe gestures (edge swipe-in opens, content swipe-out closes; release-classified A 档: zero inline transform, no backdrop, commit = `ctx.layout.toggleSidebar()`; coexists with the host overlay handlers via `gesture-guard.ts` predicates).
...
- Treat DOM markers as the cross-module state contract: `data-mobile-nav="frame"`, `data-sidebar-collapsed`, `data-aionui-explorer-open`, `data-aionui-preview-open`, `data-mobile-preview-full`, `data-mobile-nav="stats"`, `data-mobile-nav="hotspot"`.
...
- **抽屉手势层（sidebar-swipe.ts）的铁律与共存契约**：手势是「松手判定式」（A 档），判定通过纯函数 `classifySwipe`/`slidingVelocity`/`hitTestStart`（tests/sidebar-swipe.test.ts 覆盖决策表）。铁律：**零 inline transform、零手势层 backdrop、不 setPointerCapture**——提交一律 `ctx.layout.toggleSidebar()`，动画全交宿主 .28s transition（`transform:none` 不变式保持）。手势判定后必须 `markGestureConsumed(target, 1000, drawer)`（gesture-guard.ts，**链式标记到 drawer 为止**），宿主 `phone-chrome.ts` 的 `onDrawerClick`/`onDrawerPointerUp` 首行 `if (consumeIfGestured(event)) return`——否则手势 release 后浏览器合成的 click 会被宿主当 tap 再关一次抽屉、或触发 FAB/backdrop 二次 click。**不要掐死 iOS 自愈路径**：手势 up 后 marker 已同步翻转，自愈回调 `drawerOpen()` 为 false 直接 return；非手势 tap 谓词集合为空，原样自愈。关键 CSS 一行：drawer 滚动容器 `touch-action: pan-y`（不加则横滑被浏览器吃成 pan 发 pointercancel，手势全失效）。热区 `[data-mobile-nav="hotspot"]` 是 `pointer-events:none` 的纯视觉层，几何判定优先、DOM 不挂监听；`data-dsh-taskboard-active`/`data-dsh-ssh-active` 全屏接管时热区不创建。状态机兜底：pointercancel / visibilitychange(hidden) / blur / 拖拽中 aria-modal 升起 → reset。参数（2026-08-27 用户反馈"行程太长"后调优，CDP 实测 78px 开 / 64px 关）：热区 24px、slop 4px、directionBias 1.5、open 0.20 / close 0.16 视口比例、速度窗口 120ms、openVel/closeVel 0.45 px/ms（MUI 同款）、cooldown 350ms。**距离从起点（startX）起算、slop 只作激活门槛不消耗行程**——若改回从锁定点起算，有效行程变 slop+阈值（如 4+78px），用户滑到阈值距离还差一截（"要滑半个屏幕"的根因之一）。`gesture-guard.ts` 保持**零 import**（node:test 直接可测，用特征检测而非 `instanceof Element` 以便非 DOM 环境运行）。
- **手势消费标记的 backdrop 误吞坑（"点两次才关"bug，2026-08-27 修复）**：手势打开抽屉后 `markGestureConsumed` 链式标记手势起点目标，若起点 `event.target` 的祖先链**不含 drawer**（headless 命中穿透时起点落到 body；或 drawer 空壳无内容元素），链会一路走到 document——把 **backdrop / FAB 也标记为 consumed**。随后 1s 窗口内点 backdrop 想关闭，sidebar-swipe 的 document 捕获 `onClick` 命中标记 → `stopPropagation` → backdrop 元素级 click 监听收不到 → 点一次无效（用户感知"要点两次"）。修复：`onClick` 对命中 `[data-mobile-nav="backdrop"], [data-mobile-nav="fab"]` 的 click **无条件放行**（backdrop/FAB 绝不可能是手势合成 click 的目标——手势起点只在左缘热区/drawer 内容区）。同时 `markGestureConsumed` 的 upTo 收敛为 drawer（非 frame），减小误标范围。
