...
- **（同一坑的第四半，终局）键盘过渡的整卡瞬灭 = WebView 表面行为，前端杠杆三层到顶，用户拍板接受**（2026-09-27，两轮真机实验收束）。现象：恢复搜索行后（本坑第二半的修法让行回来了），点搜索框抬键盘瞬间**整卡消失 1-2 帧再回来**——几何锁全部生效（不塌、不缩），所以是「消失」形态。排除链：remount 通道全灭（宿主 shortcuts/primitives 零 resize 订阅、layout ResizeObserver 纯测量、theme matchMedia 只翻类、conversation 的 visualViewport 是滚动可见性计算）；提升源为零（`--dsw-mask-blur`=none、卡片无 transform/opacity/filter）。两刀实验：**合成提升**（PR #133 `will-change: transform`，理论依据=几何跨 resize 不变故层缓存可复用）真机**阴性**，PR #134 撤下无效声明与假锚——键盘 resize 连已提升层的栅格也一起作废，这引擎上没有前端手段能让卡片逃过键盘过渡的重光栅。终局=用户拍板 A（接受这一下），与 composer 键盘坏帧先例同判：仅抬键盘那 1-2 帧，几何/功能/明暗全对。`interactive-widget=overlays-content` 明确否决（page 级 viewport meta 契约 + composer 会被键盘盖住 + 与宿主 meta 改写互搏）。**给后人的铁律**：键盘过渡类「闪/瞬灭」先用排除链定性（几何锁有没有生效→remount 通道→提升源），定性为表面行为就停手报用户裁决，不要再造第四把前端杠杆。
