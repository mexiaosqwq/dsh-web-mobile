...
export async function deleteSession(deps, sessionId) {
    ...
    // The detach call itself gets the same best-effort guard as the failure
    // path above: the session is fully gone from the store at this point, so a
    // throwing workspace registry must not turn a finished deletion into an
    // error — a stale workspace id heals on the next host reconcile.
    try {
        await detachFromWorkspaces(deps, sessionId);
    }
    catch {
        // Same phase-1 accounting gap: the stale workspace id stays until the
        // host reconciles it.
    }
