...
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  [aria-modal="true"][data-shortcut-modal="shortcuts"] {
    ...
    transition: max-height .2s var(--ds-ease-out, ease-in-out);
    transform: none !important;
    /* 光栅隔离（2026-09-27 真机「弹层瞬间消失又回来」）：键盘 resize 触发整页
       重光栅，未提升的卡片 tile 在低端 WebView 上赶不上帧截止 = 缺席 1-2 帧再
       补上（remount 通道已全排除：宿主弹层树无任何 resize→setState 订阅）。
       will-change 把卡片提成独立合成层——几何在键盘瞬间不变（宽/top/max-height
       全锁），层缓存可跨 resize 复用，不再参与整页重光栅风暴。 */
