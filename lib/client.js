...
function installModelMenuAnchor(ctx) {
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-web-mobile: model menu anchor', () => {
        ...
        const place = (menu) => {
            ...
            const trigger = laidOut(document.querySelector(MODEL_TRIGGER));
            const card = laidOut(document.querySelector(COMPOSER_CARD));
            let left;
        ...
        const settle = () => {
            if (active !== null && laidOut(active) === null)
                active = null;
        ...
        const onPointerDown = (event) => {
            ...
            if (!(target instanceof Element))
                return;
            if (target.closest(MODEL_TRIGGER) !== null || target.closest(MODEL_MENU) !== null)
                scheduleSettle();
        };
