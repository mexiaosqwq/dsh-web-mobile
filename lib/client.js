...
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  /* --- Conversation text on mobile ---
     ...
     <p> paragraphs. Since DSH 0.1.2-rc.1 the composer is a Lexical
     contenteditable that also renders real <p> paragraphs inside its own
     _scroll container, so the composer must be excluded explicitly via
     :not(:has([data-composer-input])) — the old textarea-based exclusion
     (:has(p) alone) no longer holds. */
  ...
  /* --- Composer input on DSH >= 0.1.2-rc.1: iOS focus auto-zoom ----------
     The Lexical composer contenteditable ([data-composer-input]) inherits
     the composer card's 14px font (--dsh-content-font-size). iOS Safari —
     home-screen PWA included — enlarges the whole viewport when a focused
     text field computes font-size < 16px, and the zoom only reverts on
     blur. max() keeps any deliberately larger content font-size
     preference; desktop fine-pointer devices never enter this media
     block; pinch zoom stays available. */
  [data-composer-input] {
