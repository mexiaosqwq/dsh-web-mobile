    ...
    while (node !== null && node !== undefined && hops < limit) {
        ...
        if (props !== null && props !== undefined) {
            for (const key of OBJECT_KEYS) {
                ...
                if (hit === 'known')
                    return candidate;
                if (hit === 'plausible' && fallback === null)
                    fallback = candidate;
            }
...
function classify(value, isKnownId) {
...
function reactFiberOf(instance) {
    ...
    for (const key of Object.keys(record)) {
        if (key.startsWith('__reactFiber$') || key.startsWith('__reactInternalInstance$')) {
            ...
            return typeof fiber === 'object' && fiber !== null ? fiber : null;
        }
    }
    return null;
}
    ...
    installMobileEffect(ctx, 'dsh-web-mobile: drawer close (Escape + navigate)', () => {
        ...
        const onDrawerClick = (event) => {
            ...
            if (performance.now() - lastTouchNavAt < 500) {
                ...
                if (performance.now() - fallbackNavAt < 600)
...
exports.shouldClaimTouchMove = shouldClaimTouchMove;
 ...
 */
function shouldClaimTouchMove(locked, movedPx) {
    if (locked)
        return true;
...
function installSidebarSwipe(ctx) {
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-web-mobile: sidebar swipe gestures', () => {
        ...
        const onTouchMove = (event) => {
            ...
            const movedPx = touch === undefined
                ? 0
                : Math.max(Math.abs(touch.clientX - startX), Math.abs(touch.clientY - startY));
            if (!shouldClaimTouchMove(tracking, movedPx))
                return;
            event.preventDefault();
        };
