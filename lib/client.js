...
function classifySwipe(t, m, rtl) {
    ...
    if (Math.abs(dx) <= t.slopPx)
        return 'none';
    if (Math.abs(dx) <= t.directionBias * Math.abs(m.dy))
        return 'none';
    if (t.drawerOpen) {
        if (dx <= 0)
            return 'none';
        if (dx / t.viewportWidthPx >= t.closeDistanceRatio)
            return 'close';
...
function beginStroke(event, rtl, viewportWidthPx) {
    ...
    if (open) {
        ...
        if (drawer === null || !drawer.contains(event.target))
            return false;
        if (event.target.closest('[data-mobile-nav="backdrop"]') !== null)
            return false;
...
function tryLock(event) {
    ...
    if (Math.abs(dx) <= SLOP_PX)
        return false;
    if (Math.abs(dx) <= DIRECTION_BIAS * Math.abs(dy))
        return false;
    tracking = true;
    lockX = event.clientX;
    lockY = event.clientY;
...
function endStroke(ctx, event, rtl, viewportWidthPx) {
    ...
    // to the drawer/frame, so the host's synthetic re-dispatched click (whose
    // target is the row root) is covered too.
    const frame = (0, phone_chrome_ts_1.getFrame)();
    const markUpTo = frame ?? null;
    (0, gesture_guard_ts_1.markGestureConsumed)(event.target, CONSUME_WINDOW_MS, markUpTo);
...
function installSidebarSwipe(ctx) {
    let removeHotspotTask = null;
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-mobile-nav: sidebar swipe gestures', () => {
        const viewportWidth = () => window.innerWidth || document.documentElement.clientWidth || 0;
        removeHotspotTask = (0, phone_chrome_ts_1.addReconcilerTask)(createHotspotTask());
